...
import (
	...
	_ "github.com/emer/etable/etview"
...
type Sim struct {
	...
	ACon    int64 `view:"-" desc:"Blocked on or not"`
}
...
func (ss *Sim) New() {
	...
	ss.MemThr = 0.34
	ss.LayStatNms = []string{"ECin", "DG", "CA3", "pCA1", "dCA1"}
	ss.TstNms = []string{"AB"}
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	...
	in := net.AddLayer2D("Input", 18, 1, emer.Input)
	ecin := net.AddLayer2D("ECin", 18, 1, emer.Hidden)
	ecout := net.AddLayer2D("ECout", 18, 1, emer.Target) // clamped in plus phase
	pca1 := net.AddLayer2D("pCA1", 5, 10, emer.Hidden)
	dca1 := net.AddLayer2D("dCA1", 5, 10, emer.Hidden)
	...
	dg.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "ECin", YAlign: relpos.Front, XAlign: relpos.Middle, Space: 0})
	ca3.SetRelPos(relpos.Rel{Rel: relpos.Above, Other: "DG", YAlign: relpos.Front, XAlign: relpos.Left, Space: 0})
	dca1.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "CA3", YAlign: relpos.Front, Space: 2})
	pca1.SetRelPos(relpos.Rel{Rel: relpos.Behind, Other: "dCA1", YAlign: relpos.Front, Space: 2})
	...
	pj = net.ConnectLayersPrjn(ecin, dca1, prjn.NewFull(), emer.Forward, &hip.EcCa1Prjn{})
	pj.SetClass("EcCa1Prjn")
	pj = net.ConnectLayersPrjn(dca1, ecout, prjn.NewFull(), emer.Forward, &hip.EcCa1Prjn{})
	pj.SetClass("EcCa1Prjn")
	pj = net.ConnectLayersPrjn(ecout, dca1, prjn.NewFull(), emer.Back, &hip.EcCa1Prjn{})
	pj.SetClass("EcCa1Prjn")
	...
	// Schafer collaterals
	pj = net.ConnectLayersPrjn(ca3, pca1, path, emer.Forward, &hip.CHLPrjn{})
	pj.SetClass("HippoCHL")

	// Additional CA1
	pj = net.ConnectLayersPrjn(pca1, ecout, prjn.NewFull(), emer.Forward, &hip.CHLPrjn{})
	pj.SetClass("HippoCHL")
	pj = net.ConnectLayersPrjn(ecout, pca1, prjn.NewFull(), emer.Back, &hip.CHLPrjn{})
	pj.SetClass("HippoCHL")
...
func (ss *Sim) AlphaCyc(train bool) {
	...
	dca1FmECin := dca1.RcvPrjns.SendName("ECin").(*hip.EcCa1Prjn)
	pca1FmCa3 := pca1.RcvPrjns.SendName("CA3").(*hip.CHLPrjn)

	ecoutFmpCa1 := ecout.RcvPrjns.SendName("pCA1").(*hip.CHLPrjn)
	ecoutFmdCa1 := ecout.RcvPrjns.SendName("dCA1").(*hip.EcCa1Prjn)
	pca1fmecout := pca1.RcvPrjns.SendName("ECout").(*hip.CHLPrjn)
	dca1fmecout := dca1.RcvPrjns.SendName("ECout").(*hip.EcCa1Prjn)
	...
	if train {
		ecin.Inhib.Layer.Gi = 2
		ecout.Inhib.Layer.Gi = 2
		dg.Inhib.Layer.Gi = 24
		ca3.Inhib.Layer.Gi = 4.5
		pca1.Inhib.Layer.Gi = 3
		dca1.Inhib.Layer.Gi = 2.2
		pca1fmecout.WtScale.Abs = 1
		dca1fmecout.WtScale.Abs = 1
	}
	...
	if !train {
		...
		dca1FmECin.WtScale.Abs = 1
		pca1FmCa3.WtScale.Abs = 1
		ecoutFmdCa1.WtScale.Abs = 4 // MSP-strong version on. Change to 1 for TSP Strong.
		ecoutFmpCa1.WtScale.Abs = 1 // Change to 4 for TSP Strong.
	}
	...
	if train {
		dca1FmECin.WtScale.Abs = 1
		pca1FmCa3.WtScale.Abs = 0
		ecoutFmdCa1.WtScale.Abs = 1
		ecoutFmpCa1.WtScale.Abs = 0
	}
	...
	for qtr := 0; qtr < 4; qtr++ {
		...
		case 1: // Second, Third Quarters: CA1 is driven by CA3 recall
			if train {
				dca1FmECin.WtScale.Abs = 0
				pca1FmCa3.WtScale.Abs = 1
				ecoutFmdCa1.WtScale.Abs = 0
				ecoutFmpCa1.WtScale.Abs = 1
...
func (ss *Sim) TrainTrial() {
	...
	if chg {
		...
		if ss.TrainEnv.Epoch.Cur == 15 && ss.ACon == 0 {
			ss.TrainEnv.Table = etable.NewIdxView(ss.TrainAC)
			ss.ACon = 1
		}
...
func (ss *Sim) NewRun() {
	...
	pca1 := ss.Net.LayerByName("pCA1").(*leabra.Layer)
	ca3 := ss.Net.LayerByName("CA3").(*leabra.Layer)
	dg := ss.Net.LayerByName("DG").(*leabra.Layer)
	...
	pjca3ca1 := pca1.RcvPrjns.SendName("CA3").(*hip.CHLPrjn)
	pjca3ca1.Pattern().(*prjn.UnifRnd).RndSeed = ss.RndSeed
	pjca3ca1.Build()
...
func (ss *Sim) OpenPats() {
	ss.OpenPat(ss.TrainAB, "train_set1_0.5training.txt", "TrainAB", "AB Training Patterns")
	ss.OpenPat(ss.TrainAC, "train_set2_0.5training.txt", "TrainAC", "AC Training Patterns")
	ss.OpenPat(ss.TestAB, "test_random.txt", "TestAB", "AB Testing Patterns")
}
